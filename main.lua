...
_G.key = "1947942e91934003200.BHET-HPCI-QULI-OPQO"

-- You can change general settings ingame.
_G.enabled = false -- Right shift.
_G.antiparry = false -- Right ctrl.
_G.stompaura = false -- Right alt.
_G.drawlines = false -- Delete.
_G.range = 15 -- K = Up, J = Down.
_G.autohit = false -- Page up.

-- Farming features
_G.autospawn = false -- Automatically respawns you.
_G.followclosest = false -- Walks after the closest player.
_G.autoequip = false -- Automatically equips your weapon.
_G.autojump = false -- Automatically jump every few seconds.
_G.boostws = false -- Boosts your walk speed every few seconds.

-- To change these settings, run a script with only the setting you want to change.
-- Example: _G.usehitbox = 15
_G.killsay = false -- Automatically say a random sentence when getting a kill/assist.
_G.usehitbox = 10 -- How many hitboxes are used for killaura, more hitboxes = more lag. I recomend keeping it low but over stay 5.

-- Extra
_G.esp = false
_G.usecustomaudio = true -- Load the game with meme sounds.

validKeys = {
	"194794291934003200.BHET-HPCI-QULI-OPQO",
	"345847783422558209.GKNA-ZXBE-GLMH-RQRC",
}
...
for i, v in ipairs(validKeys) do
	print("Trying key.")
	wait()
	if _G.key == validKeys[i] then
		...
		local tableOfShit = {"you can force me to say anything with the command: -cnsay","if youre not convinced im a bot, you can make me say stuff LOL, say: -cnsay your message here","if youre not convinced im a bot, you can make me say stuff LOL, say: -hbsay your message here","why am i writing these messages when i can continue making the script better?! 🤣🤣😂🤣😂🤣🤣","do you ever have problems with light users parrying your ds??? get combat noodle! it has a great anti parry that works! (sometimes..)","get combat noodle!!! wait its not even released yet...","poop!!!🤣🤣😂🤣🤣🤣🤣😂🤣🤣🤣🤣😂🤣🤣🤣🤣😂🤣🤣🤣🤣😂🤣🤣","oh im sorry, did i killaura you? i can whitelist you if you want boo 🥺🥺🥺🥺","stop insulting my bot!!!😭😭😭😭 its not done yet!!! 😭😭","pee!!! 🤣🤣😂🤣🤣🤣🤣😂🤣🤣🤣🤣😂🤣🤣🤣🤣😂🤣🤣🤣🤣😂🤣🤣","🤣🤣😂🤣🤣","if you didnt realise 🤣🤣😂🤣🤣 this is a bot dont bother insulting it 🤣🤣😂🤣🤣","hey ".. randomPlayer() ..", is your dad spiderman? cause he far from home.","EZ EZ EZ EZ EZ","are you even trying?? lulululul","damn bro you got the whole squad cringing","omg ".. randomPlayer() .. " im your biggest fan!!!","how are you that bad??🤣🤣😂🤣🤣","dont even bother insulting me 🤣🤣😂 im not even touching my keyboard!!","oh god... not ".. randomPlayer() .." again...","OMG!! IS THAT THE FAMOUS ".. randomPlayer() .. "???","vex really thinks this is gonna work...", "haha got you!!!", "who the hell is vex? oh the creator of this crap ai?", "get reck't noobie", "combat noodle on top!!!", "project hook has some pretty cool features", "samuel you bozo your script is patched!!!", "😭 oh no! 😭😭 did i 😭😭 kill your mans?? 😭😭😭😭", "you: 😭😭 noooo i got murdered by a lvl ".. game.Players.LocalPlayer.leaderstats.Level.Value.."!!!", "oh my god im so sorry for killing you 😭😭😭😭😭😭😭", "oh 😭😭 did i hurt your 😭😭 feelings???", "get good with project hoo- i mean with combat noodle!!!", "samuel no one cares about criminality!!!😭😭", "who even wrote this crappy code??😭", "this script was brought to you by raid shadow legends!!", "do you like cheese?", "😭😭 imagine 😭😭 dying 😭😭", "get killaura'd!!!", "oh no 😭😭 did i kill your 😭😭 girl?? 😭😭😭"}
		...
		for i,v in pairs(game.Players:GetPlayers()) do
			v.Chatted:connect(function(msg)
				if string.sub(msg, 1, 6) == "-cnsay" then
					game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer("👌:troll: " .. v.Name .. " :troll:👌","All")
				end
			end)
		end
					...
					if v.Name == "DmgPoint" then
						...
						if _G.antiparry == true then
							if theirHrp.Parent.SemiTransparentShield.Transparency == 1 then
									...
									for i,s in pairs(game.Workspace.PlayerCharacters[game.Players.LocalPlayer.Name].Stomp.Hitboxes.RightLegHitbox:GetChildren()) do
										if s.Name == "DmgPoint" then
											if i <= _G.usehitbox then
												...
												if hittable == 100 then
													if _G.autohit == true then
														mouse1click()
													end
													hittable = 0
												end
												...
												if _G.enabled == true then
													setAttachmentWorldCFrame(s, CFrame.new(game.Workspace.PlayerCharacters[game.Players.LocalPlayer.Name]:FindFirstChild("Head").Position + Vector3.new(0, 10, 0)))
												end
											end
										end
									end
								...
								game.Workspace.PlayerCharacters[game.Players.LocalPlayer.Name]:FindFirstChildOfClass("Tool").Parent = game.Workspace.PlayerCharacters[game.Players.LocalPlayer.Name].Backpack
								if _G.enabled == true then
								...
								repeat wait() until theirHrp.Parent.SemiTransparentShield.Transparency == 1
								game.Workspace.PlayerCharacters[game.Players.LocalPlayer.Name].Backpack:FindFirstChildOfClass("Tool").Parent = game.Workspace.PlayerCharacters[game.Players.LocalPlayer.Name]
							end
						else
							if _G.enabled == true then
								setAttachmentWorldCFrame(v, CFrame.new(theirHrp.Position + Vector3.new(math.random(-1,1),math.random(-1,1),math.random(-1,1))))
		...
		credits.Text = "Combat hook - IsaaaKK"
		...
		if totalFails == #validKeys then
			...
			StarterGui:SetCore("SendNotification", {
				Title = "Invalid key!",
				Text = "Please make sure you copied your entire key!",
				Duration = 5,
				Callback = bindable,
				Button1 = "Ok!"
			})
			...
			sound1.SoundId = "rbxassetid://7130219477"
			sound1.Volume = 1
			sound1:Play()
			...
			sound.SoundId = "rbxassetid://8204383614"
			sound.Volume = 2
			sound:Play()
		end
	end
